...
public class TransferService {
  ...
  private void makeTransfer(Transfer transfer) {
    accountService.plusSum(transfer.getToAccountId(), transfer.getSum());
    accountService.minusSum(transfer.getFromAccountId(), transfer.getSum());
  }

  private void rollbackTransfer(Transfer transfer) {
    accountService.plusSum(transfer.getFromAccountId(), transfer.getSum());
    accountService.minusSum(transfer.getToAccountId(), transfer.getSum());
  }
  ...
  private void oldTransferToNewTransferMapping(Transfer oldTransfer, Transfer newTransfer) {
    ...
    if (newTransfer.getFromAccountId() == null) {
      newTransfer.setFromAccountId(oldTransfer.getFromAccountId());
    }
    if (newTransfer.getToAccountId() == null) {
      newTransfer.setToAccountId(oldTransfer.getToAccountId());
    }
